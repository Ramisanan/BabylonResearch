"""CLI: run Benford's Law (first-digit) analysis on the Pirngruber price
data (Data/BabylonianPrices.xlsx) -- same X (quantity per shekel) / 1/X
(shekel per unit) methodology as run_benford.py, same schema as the
Vanderspek workbook but with English "-interpretation" column names and
more rows -- and save the results to
"Benford Law Data Output for Pirngruber Data/".

Usage:
    python scripts/run_benford_pirngruber.py
"""
...
from babylon_prices import convert_x_to_1_over_x, digit_distribution, conformity_stats
from babylon_prices.io import load_dataset

ROOT = Path(__file__).resolve().parent.parent
SRC = ROOT / "Data" / "BabylonianPrices.xlsx"
OUT_DIR = ROOT / "Benford Law Data Output for Pirngruber Data"
