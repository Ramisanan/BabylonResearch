...
-- for both the X and 1/X formats. Each commodity's mean/std/CDF is
computed only from that commodity's own values -- commodities are never
mixed together, and X values are never inverted to get 1/X (both are
read from source columns independently).
...
into "Zscore Normalized Data Output for Vanderspek Data/".

Usage:
    python scripts/zscore_normalize.py
"""
...
from babylon_prices import COMMODITIES, convert_x_to_1_over_x, zscore_normalize, empirical_cdf
